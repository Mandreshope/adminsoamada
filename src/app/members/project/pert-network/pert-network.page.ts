...
import * as pert from 'pert-estimate'
import * as go from "gojs";
import { GoJsComponent } from 'src/app/components/goJs/go-js/go-js.component';

@Component({
  selector: 'app-pert-network',
  templateUrl: './pert-network.page.html',
  styleUrls: ['./pert-network.page.scss'],
  entryComponents: [GoJsComponent]
})
export class PertNetworkPage implements OnInit {
  projectId: string
  project: any
  timeline: any
  progression: number
  ...
  ngOnInit() {
    ...
    // let aNoms = ["Nom1","Nom2","Nom3"]
    // let aPrenoms = ["Prenoms1","Prenoms2","Prenoms3"]
    // let tab = [aNoms, aPrenoms]
            ...
            resp.data.forEach(e => {
              if (res.data._id == e._id) {
                ...
                if (this.progression == 1) {
                  console.log('progression == 1 ' + this.progression)
                  let data = {
                    developpement: 'Fait',
